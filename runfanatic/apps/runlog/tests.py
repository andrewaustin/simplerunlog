...
class ModelTest(TestCase):
    ...
    def test_run_minutes_errors_if_invalid(self):
        ...
        aRun = Run(user = self.user, date = "2011-7-21", hours = 0, minutes = 60, seconds = 17, distance = 4)
        self.assertRaises(ValidationError, aRun.full_clean)
    ...
    def test_run_seconds_errors_if_invalid(self):
        ...
        self.assertRaises(ValidationError, aRun.full_clean)
        aRun = Run(user = self.user, date = "2011-7-22", hours = 0, minutes = 19, seconds = 60, distance = 4)
        self.assertRaises(ValidationError, aRun.full_clean)
...
class ViewsTest(TestCase):
    ...
    def test_calculates_six_week_avg_properly(self):
        ...
        NOW = datetime.now()
        Run1 = Run(user = self.user, date = NOW, hours = 0, minutes =
                19, seconds = 1, distance = 7)
        Run1.save()
        Run2 = Run(user = self.user, date = NOW-timedelta(days=7), hours = 0,
                minutes = 19, seconds = 1, distance = 11)
        Run2.save()
        Run3 = Run(user = self.user, date = NOW-timedelta(days=14), hours = 0,
                minutes = 19, seconds = 1, distance = 23)
        Run3.save()
        Run4 = Run(user = self.user, date = NOW-timedelta(days=21), hours = 0,
                minutes = 19, seconds = 1, distance = 1)
        Run4.save()
        Run5 = Run(user = self.user, date = NOW-timedelta(days=28), hours = 0,
                minutes = 19, seconds = 21, distance = 18)
        Run5.save()
        Run6 = Run(user = self.user, date = NOW-timedelta(days=42), hours = 0,
                minutes = 19, seconds = 38, distance = 6)
        Run6.save()
        #Create a run that shouldn't be averaged.
        Run7 = Run(user = self.user, date = NOW-timedelta(days=43), hours = 0,
                minutes = 19, seconds = 49, distance = 400)
        Run7.save()
        self.c.login(username='user', password='pass')
    ...
    def test_unvalid_add_returns_errors(self):
        self.c.login(username='user', password='pass')
        response = self.c.post('/add/', {'user': self.user, 'date' : '2011-7-22', 'hours' :
            0, 'minutes': 19, 'distance':4})
